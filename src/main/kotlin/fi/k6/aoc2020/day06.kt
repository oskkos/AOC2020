...
fun day06() {
    ...
    println("Part 1: " + day0601(answerPerGroup()))
    println("Part 2: " + day0602(answerPerGroup()))
 ...
 */
private fun day0601(answers: List<List<String>>) =
    answers.map { a -> a.joinToString("").toCharArray().distinct() }.fold(0, { acc, list -> acc + list.size })
 ...
 */
private fun day0602(answers: List<List<String>>): Int {
    return answers.fold(0, { acc, groupAnswers ->
        var match = 0
        val personsInGroup = groupAnswers.size
        val allAnswersForGroup = groupAnswers.joinToString("").toCharArray()

        for (answer in groupAnswers[0]) {
            if (allAnswersForGroup.count { it == answer } == personsInGroup) {
                match++
            }
        }
        acc + match
...
private fun answerPerGroup(): List<List<String>> {
    val input =
        """elmcifawqgkp
dynwslejfcrkpv

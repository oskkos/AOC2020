...
data class PasswordCandidate(private val num1: Int, private val num2: Int, private val key: Char, private val password: String) {
    companion object {
        fun fromString(line: String): PasswordCandidate {
            ...
            val range = splitted[0].split("-")
            val key = splitted[1].replace(":", "").toCharArray()[0]
            val password = splitted[2]
            return PasswordCandidate(range[0].toInt(), range[1].toInt(), key, password)
        }
    }
    fun isValidCandidateByCharOccurences(): Boolean {
        val hits = password.count { c -> c == key }
        return hits in num1..num2
    }
    fun isValidCandidateByCharPositions(): Boolean {
        val characters = password.toCharArray()
        return (characters[num1 - 1] == key).xor(characters[num2 - 1] == key)
    }
}

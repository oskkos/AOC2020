...
fun traverser(map: List<String>, right: Int, down: Int): Int {
    ...
    while (coords.second < map.lastIndex) {
        coords = Pair(coords.first + right, coords.second + down)
        val rowInMap = map[coords.second]
        val positionInRow = coords.first.rem(rowInMap.length)
        if (rowInMap.toCharArray()[positionInRow] == '#') trees++
    }
    return trees
}

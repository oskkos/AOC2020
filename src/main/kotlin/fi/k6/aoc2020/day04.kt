...
fun hasValidContent(passport: Map<String, String>): Boolean {
    ...
    if (passport.get("byr")!!.toInt() !in 1920..2002) {
        return false
    }
    if (passport.get("iyr")!!.toInt() !in 2010..2020) {
        return false
    }
    if (passport.get("eyr")!!.toInt() !in 2020..2030) {
        return false
    }
    if (!Regex("(1[5-8][0-9]|19[0-3])cm|(59|6[0-9]|7[0-6])in").matches(passport.get("hgt")!!)) {
        return false
    }
    if (!Regex("#(\\d|[a-f]){6}").matches(passport.get("hcl")!!)) {
        return false
    }
    if (!listOf("amb", "blu", "brn", "gry", "grn", "hzl", "oth").contains(passport.get("ecl"))) {
        return false
    }
    if (!Regex("\\d{9}").matches(passport.get("pid")!!)) {
        return false
    }
    return true
}
...
fun passports(): List<Map<String, String>> {
    val input =
        ...
        hgt:183cm cid:187 byr:2019 ecl:xry iyr:2013 pid:164cm hcl:#18171d eyr:2021""".lines().map { it.trim() }
    ...
    input.forEach { row ->
        ...
        for (chunk in chunks) {
            val splitted = chunk.split(":")
            passport.set(splitted[0], splitted[1])
        }
    }
    passports.add(passport)
    return passports
